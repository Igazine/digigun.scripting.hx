...
class LibRun {
	static public function main() {
		final args = Sys.args();
		if (args.length < 1) {
			Sys.println('Usage: haxelib run digigun.scripting.hx <command> <input>');
			Sys.exit(1);
		}
		...
		switch command.toLowerCase() {
			case 'bc':
				try {
					bytecodeCompile(workingDir, args.shift(), args.shift());
				} catch (e:Dynamic) {
					Sys.println('Compilation error: ${e}');
					Sys.exit(1);
				}
			default:
				Sys.println('Unknown command\nUsage: haxelib run digigun.scripting.hx <command> <input> <output>');
				Sys.exit(1);
		}
	}
	...
	static function collectImports(e:Expr, imports:Array<String>) {
		...
		switch (e.def) {
			...
			case EBlock(exprs):
				for (expr in exprs) collectImports(expr, imports);
			case EIf(_, e1, e2):
			...
			case ESwitch(_, cases, defExpr):
				for (c in cases) collectImports(c.expr, imports);
				collectImports(defExpr, imports);
			case ETry(tryExpr, catches):
				collectImports(tryExpr, imports);
				for (c in catches) collectImports(c.body, imports);
			default:
		}
	}

	public static function bytecodeCompile(workingDir:String, input:String, ?key:String) {
		if (input == null) {
			throw 'Usage: haxelib run digigun.scripting.hx bc <input> <key>';
		}
		...
		if (FileSystem.isDirectory(fullInputPath)) {
			...
			for (file in filesToCompile) {
				...
				try {
					compileSingleFile(workingDir, relFilePath, key);
				} catch (e:Dynamic) {
					Sys.println('Failed to compile ${relFilePath}: ${e}');
				}
			}
		} else {
			compileSingleFile(workingDir, input, key);
		}
	}

	static function compileSingleFile(workingDir:String, input:String, ?key:String) {
		var fullInputPath = workingDir + input;
		...
		var modules = new Map<String, { fqName:String, path:String, ast:Expr, dependencies:Array<String> }>();
